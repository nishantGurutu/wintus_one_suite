...
class NoteEditorScreen extends StatefulWidget {
  final int folderId;
  const NoteEditorScreen({super.key, required this.folderId});
...
class _NoteEditorScreenState extends State<NoteEditorScreen> {
  ...
  dispose() {
    super.dispose();
    notesController.profilePicPath.value = '';
  }
        ...
        children: [
          ...
          Obx(
            () {
              return Column(
                children: [
                  if (notesController.profilePicPath.value.isNotEmpty)
                    InkWell(
                      onTap: () {
                        openFile(File(notesController.profilePicPath.value));
                      },
                      child: Container(
                        height: 150.h,
                        width: 250.w,
                        ...
                        child: ClipRRect(
                          ...
                          child: Image.file(
                            File(notesController.profilePicPath.value),
                            fit: BoxFit.cover,
  ...
  Future<void> takePhoto() async {
    try {
      ...
      if (result != null && result.files.isNotEmpty) {
        ...
        notesController.pickedFile.value = File(file.path);
        notesController.profilePicPath.value = file.path.toString();
        print(
            'selected file path from device is ${notesController.pickedFile.value}');
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
